class EchoManager{
	constructor(region,parent,hard=false){
		this.parent=parent;
		this.region=region;
		this.properties_class=null;
		this.loaded_area_indexes=[true];
		this.hard=hard;
	}
	create_areas(map_new_areas,index,area_class){
		if(typeof this.region == "string")this.region=this.parent.regions.filter(e=>e.name==this.region)[0];
		let new_areas = this.check_index(index,area_class)
		for(let new_area in new_areas){
			map_new_areas.push(new_area)
		}
	}
	check_index(index,area_class){
		let result=[];
		let _check_index=(function(index){
			if(!this.loaded_area_indexes[index]){
				this.loaded_area_indexes[index]=true;
				this.region.areas[index]=this.generate_area(index);
			}
		}).bind(this);
	...
	generate_area(index,area_class){
		let area_x_position = (!this.hard?(3200*index+1952):(-3200*index-1248)),
		area_y_position=23200,area_width=3200,area_height=480,
		...
		if(this.hard)zone_dicts.reverse();
		let zones=[];
		for(let zone_dict of zone_dicts){
			...
			zones.push(zone_definition);
		}
		let area_properties;
		if(this.hard){
			area_properties={background_color:[55,94,186,75]};
		}
		let new_area=new Area(
			area_x_position,area_y_position,
			[],
			[],
			area_properties,
			this.region
		);
		zones.map(e=>new_area.zones.push(new Zone(e.type,e.x,e.y,e.width,e.height,e.properties,e.spawner,e.translate,this.region,index)));
		new_area.bounds=new_area.findBounds();
		return new_area;
	}
}
...
class RandomEnemyGenerator{
	constructor(area,hard=false){
		...
		this.area_index=area.parent.areas.indexOf(area);
		this.hard=hard;
		//this.seed=`${this.area_index}:${this.hard}`
	...
	generate_random_enemies(area_index,zone,get_new_id,generate_enemies,generate_wall_enemies,entity_config_class){
		...
		var activeZone=this.area.zones.filter(e=>e.type=="active")[0];
		if(!activeZone)return;
		activeZone.spawner=[];
		if(area_style=="normal"){
			let group_count=choose(this.possible_group_counts,Math.random);
			if(area_index > 199){
				activeZone.spawner.push(this.generate_ghost_group(area_index,entity_config_class));
			}
			...
			activeZone.spawner.push({types:[enemy_type],count:4,radius:30*this.boss_radius_multiplier_function(area_index),speed:boss_speed})
			for(let group of this.generate_group(
				area_index,
				1,
				this.minor_enemy_types,
		...
		if(area_index > 1){
			activeZone.spawner.push({types:["wall"],count:6,speed:Math.min(2/3*(6+(1+area_index/40)*Math.sin(area_index/3)+area_index/20),26),radius:30})
		}
	}
	generate_group(area_index,group_count,enemy_types,entity_config_class,count_multiplier,radius_multiplier,speed_multiplier){
		...
		groups.push({types:gd.enemy_types,count:gd.count,radius:gd.radius,speed:gd.speed});
		if(gd.extra_normal_enemy_count > 0){
			groups.push({types:["normal"],count:gd.extra_normal_enemy_count,radius:gd.radius,speed:gd.speed});
		}
		return groups;		
	}
	generate_ghost_group(area_index,entity_config_class){
		...
		return {
			types:[enemy_type],
			count:12,
			radius:Math.min(((area_index-200)/8)+5,80)*ghost_type_compensation_coeff,
			speed:Math.min(((area_index-200)/24)*5,30)*ghost_type_compensation_coeff
		}
	}
	speed_function(x){
		if(this.hard&&x>20&&(this.area.properties.magnetism||this.area.properties.partial_magnetism))
			return 0.5*2/3*Math.log2(x**2+1)+x/40+2;
		return 2/3*Math.log2(x**2+1)+x/40+2;
	}
	count_function(x){
		if(this.hard&&(this.area.properties.magnetism||this.area.properties.partial_magnetism))
	...
	enemy_type_offset_handler(area_index){
		if(this.hard)return max(Math.ceil(area_index*0.8-10),0)
		return area_index;
	}
